...
def _ensure_callback_users_schema():
    ...
    # Email is not unique — only mobile is. Drop legacy unique indexes on email.
    with db.engine.begin() as conn:
        index_rows = conn.execute(text(
            "SELECT name, sql FROM sqlite_master "
            "WHERE type='index' AND tbl_name='callback_users' AND name IS NOT NULL"
        )).fetchall()
        for index_name, index_sql in index_rows:
            if not index_sql:
                continue
            normalized = index_sql.lower()
            if "unique" in normalized and "email" in normalized and "mobile" not in normalized:
...
def _post_lead_to_nopaperforms(
    *,
    ...
    payload = {
        ...
        "cf_form_name": "Landing Page - KMPL-SO-DST",
        "cf_platform": "Website",
        "cf_program": "KMPL-SO-DST",

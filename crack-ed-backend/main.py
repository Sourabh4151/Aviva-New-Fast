...
def _ensure_callback_users_schema():
    ...
    if "sales_experience" not in columns:
        migrations.append("ALTER TABLE callback_users ADD COLUMN sales_experience VARCHAR(50)")
    if migrations:
...
OTP_ELIGIBLE_SALES_EXPERIENCE_OPTIONS = (
    "1–3 years",
    "More than 3 years",
)


def _normalize_sales_experience(value):
    raw = (value or "").strip()
    return raw if raw in VALID_SALES_EXPERIENCE_OPTIONS else ""
...
def send_callback_lead_to_crm(user):
    ...
    response = _post_lead_to_nopaperforms(
        ...
        sales_experience=getattr(user, "sales_experience", "") or "",
    )
...
def send_callback_otp():
    ...
    if sales_experience_value not in OTP_ELIGIBLE_SALES_EXPERIENCE_OPTIONS:
        return jsonify({
            "error": "At least 1 year of Collection/Recovery experience is required to apply"
        }), 400
    ...
    def _apply_profile_fields(target):
        try:
            ...
            target.sales_experience = sales_experience_value
        except AttributeError:
            pass

...
def main():
    ...
    for row in rows:
        ...
        for idx, field in enumerate(file_fields):
            ...
            if len(parts) != 2:
                continue
            old_abs_path = os.path.join(UPLOADS_ROOT, os.path.relpath(file_path, 'uploads'))
            new_path = f"{parts[0]}/loan_application/{applicant_folder}/{parts[1]}"
            new_abs_path = os.path.join(UPLOADS_ROOT, os.path.relpath(new_path, 'uploads'))
    ...
    conn.commit()
    print(f"Moved and updated {len(updates)} file paths.")
    for app_id, field, old, new in updates:
        print(f"MOVED: application_id={app_id}, field={field}, old={old}, new={new}")
    if not updates:

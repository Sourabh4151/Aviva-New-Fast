...
export default function RequestCallbackModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setForm({ name: "", email: "", state: "", city: "", mobile: "" });
    setStatus(null);
    setErrors({});
    setConsent(false);
    setStateOpen(false);
  ...
  useEffect(() => {
    ...
    function onKeyDown(e) {
      if (e.key === "Escape" && !showPopup) onClose();
    }
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [isOpen, onClose, showPopup]);
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4 py-6 overflow-y-auto"
        onClick={onClose}
        role="presentation"
      >
        <aside
          ...
          <div>
            ...
            >
              ...
              <div ref={stateRef} className="relative">
                ...
                {errors.state && <p className="mt-1 text-[12px] text-red-400">{errors.state}</p>}
              </div>
              <div ref={cityRef} className="relative">
                ...
                {errors.city && <p className="mt-1 text-[12px] text-red-400">{errors.city}</p>}
              </div>
              {!showOtp && (
                <>
                  <input
                    ...
                    className="w-full px-4 h-[50px] rounded-[10px] bg-transparent border border-[rgba(250,250,250,0.3)] outline-none focus:outline-none focus:ring-0 focus:border-[rgba(250,250,250,0.55)] callback-input"
                  />
                  {errors.mobile && <p className="mt-1 text-[12px] text-red-400">{errors.mobile}</p>}
                  {!alreadyInSystem && (
              ...
              {showOtp && (
                <>
                  ...
                  {otpError && <p className="mt-2 text-[12px] text-red-400">{otpError}</p>}
                </>
              )}
            </form>
          </div>
